...
def send_lert_to_home():
    cv.imwrite("patientImage.jpg", debug_image)
    pywhatkit.sendwhats_image(
        "+8801575056952", "patientImage.jpg", "I am not feeling good. Come fast please."
    )


def send_data_to_arduino(message):
    serialIns.write(message.encode("utf-8"))
    print(message)


# Function to calculate distance between two points
def calculate_finger_distance(firstParameter, secountParameter):
    return np.sqrt(
        (firstParameter[0] - secountParameter[0]) ** 2
        + (firstParameter[1] - secountParameter[1]) ** 2
    )
...
def draw_landmarks(image, landmark_point):
    if len(landmark_point) > 0:
        ...
        """'
        TODO: Finger distance show
        cv.line(
            image, tuple(landmark_point[4]), tuple(landmark_point[4]), (0, 0, 0), 6
        )
        cv.line(
            image,
            tuple(landmark_point[4]),
        ...
        """
        print(landmark_point[8][1])
        # print(int("".join(map(str, landmark_point[0]))))

        if (
            landmark_point[8][1] <= 400
            and landmark_point[12][1] >= 350
            and landmark_point[18][1] >= 350
        ):
            print("volumeLevel control")
            thumb_and_index_finger_distance = calculate_finger_distance(
                landmark_point[4], landmark_point[8]
            )
            volumeLevel = np.interp(
                thumb_and_index_finger_distance, [30, 200], [0, 100]
            )
            set_system_volume(int(volumeLevel))

        # if landmark_point[4].x < [landmark_point[4] - 1 ].x:
        #     print("Finger open")
        # else:
        #     print("finger closed")
...
def draw_info_text(image, brect, handedness, hand_sign_text, finger_gesture_text):
    ...
    if hand_sign_text == "help" and sendMessage == 0:
        send_lert_to_home()
